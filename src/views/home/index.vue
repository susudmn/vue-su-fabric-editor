    ...
    <div class="home">
      <Layout>
        ...
        <Header v-if="state.show">
          <!-- logo -->
          <span class="logo">
            <a href="https://github.com/susudmn" target="_blank">
        ...
        <Tooltip :content="$t('grid')">
            <Switch size="small" v-model="state.ruler" @on-change="rulerSwitch" />
        </Tooltip>
        <Divider type="vertical" />
        <history></history>
      ...
      <Content style="display: flex; height: calc(100vh - 45px);">
        <!-- 左侧区域 -->
        <div v-if="state.show" :class="`left-bar ${state.toolsBarShow && 'show-tools-bar'}`">
          <Menu :active-name="state.menuActive" accordion @on-select="showToolsBar" width="65px">
              <MenuItem to="/template" :name="1" class="menu-item">
                <Icon type="md-book" size="24" />
                <div>{{ $t('templates') }}</div>
              </MenuItem>
              <MenuItem to=/element :name="2" class="menu-item">
              <Icon type="md-images" size="24" />
              <div>{{ $t('elements') }}</div>
              </MenuItem>
              <MenuItem to="/cartoon" :name="3" class="menu-item">
              <Icon type="ios-leaf-outline" size="24" />
              <div>{{ $t('material.cartoon') }}</div>
              </MenuItem>
              <MenuItem to='/layer' :name="4" class="menu-item">
        ...
        <div id="workspace">
          <div class="canvas-box">
            ...
            <canvas width="923" height="573" id="canvas" :class="state.ruler ? 'design-stage-grid' : ''"></canvas>
