<template>
    <div>
        <Divider orientation="left" style="font-size: 14px;color: #5e5c5c;">{{ $t('common_elements') }}</Divider>
         <div class="tool-box">
        ...
        <Divider orientation="left" style="font-size: 14px;color: #5e5c5c;">{{ $t('draw_elements') }}</Divider>
        <div class="tool-box">
          <span
...
onMounted(() => {
  nextTick(() => {
    ...
    console.log(offset.left); //380
      // 鼠标坐标转换成画布的坐标（未经过缩放和平移的坐标）
      const point = {
        // 元素距离左侧(顶部)的距离 - 画布距离左侧(顶部)的距离
        x: opt.e.x - offset.left,
        y: opt.e.y - offset.top,
      };
      console.log(opt.e.x); // 646
